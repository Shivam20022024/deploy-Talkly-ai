...
import { Mail, Lock, ArrowRight, Sparkles, ShieldCheck, Github } from 'lucide-react';
...
const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Simulate API call
    setTimeout(() => {
      if (email === 'admin@talkly.ai' && password === 'admin') {
        login({
          id: 'emp_001',
          name: 'Shivam',
          email: 'shivam@talkly.ai',
          role: 'Administrator',
          avatar: 'https://ui-avatars.com/api/?name=Shivam&background=059669&color=fff'
        });
        onLoginSuccess();
      } else {
  ...
  return (
    <div
      className="min-h-screen flex items-center justify-center font-sans selection:bg-blue-500/30 selection:text-blue-900 overflow-hidden relative"
      style={{
    ...
    >
      ...
      >
        <div className="bg-white/60 backdrop-blur-xl rounded-3xl border border-white/40 shadow-2xl shadow-blue-900/5 p-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-3.5 bg-white border border-slate-200 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm"
                  required
                />
              </div>
            </div>
            ...
            {error && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className="bg-red-50 text-red-600 p-3 rounded-lg text-xs font-medium border border-red-100"
              >
                {error}
              </motion.div>
            )}
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3.5 rounded-xl text-sm font-bold shadow-lg shadow-blue-600/20 transition-all flex items-center justify-center gap-2 group disabled:opacity-70 disabled:cursor-not-allowed"
            >
              {isLoading ? (
          ...
          <div className="mt-8 pt-6 border-top border-slate-100">
            <div className="relative flex justify-center text-xs uppercase font-bold text-slate-400 mb-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <button className="flex items-center justify-center gap-2 py-3 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors text-sm font-medium text-slate-700">
        ...
        <div className="flex justify-center mt-10">
          <p className="text-[10px] font-bold text-slate-400 flex items-center gap-1.5 border border-black/5 px-3 py-1.5 rounded-full bg-white/40 backdrop-blur-sm">
