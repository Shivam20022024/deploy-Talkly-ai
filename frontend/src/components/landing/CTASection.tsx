...
export const CTASection = ({ onStart }: { onStart?: () => void }) => {
  return (
    <section className="relative py-12 sm:py-20 md:py-30 px-4 md:px-8 bg-gray-50 dark:bg-[#0A0A1C] overflow-hidden">
      ...
      <motion.div
        ...
        style={{ background: 'radial-gradient(circle, rgba(219,183,242,0.07) 0%, rgba(122,102,138,0.03) 40%, transparent 70%)' }}
      ...
      <div className="absolute top-1/2 left-0 right-0 -translate-y-1/2 h-px">
        <div className="max-w-2xl mx-auto h-full bg-gradient-to-r from-transparent via-purple-300/30 dark:via-[#DBB7F2]/15 to-transparent" />
      </div>
      ...
      <div className="max-w-4xl mx-auto relative z-10 text-center">
        ...
        >
          ...
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#9B72C5] via-[#C49EE0] to-[#9B72C5] dark:from-[#DBB7F2] dark:via-[#EDDBF9] dark:to-[#DBB7F2] bg-[length:200%_auto] animate-[shimmer_3s_linear_infinite]">
        ...
        >
          {['No credit card required', 'Free 14-day trial', 'Setup in 5 minutes'].map((text, i) => (
            <span key={i} className="flex items-center gap-2 text-xs text-gray-400 dark:text-gray-600 font-medium">
              <span className="w-1 h-1 rounded-full bg-purple-300 dark:bg-[#DBB7F2]/30" />
              {text}
            </span>
          ))}
        </motion.div>
