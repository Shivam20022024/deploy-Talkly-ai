...
export const Sidebar = ({ onClose }: SidebarProps) => {
  ...
  return (
    <aside className="w-60 h-full bg-gray-50 dark:bg-[#0A0810] border-r border-gray-200 dark:border-white/5 flex flex-col flex-shrink-0 text-gray-500 dark:text-gray-400 overflow-y-auto overflow-x-hidden custom-scrollbar relative">
      {/* Mobile Close Button */}
      {onClose && (
        <button 
          onClick={onClose}
      ...
      <div className="p-4 flex items-center gap-3 mt-1 pr-12 lg:pr-4">
        <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#DBB7F2] to-[#7A668A] flex items-center justify-center flex-shrink-0">
          <span className="text-white dark:text-[#0A0810] font-black text-sm">T</span>
        </div>
        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            <span className="text-gray-900 dark:text-white font-bold text-base tracking-tight">TalklyAI</span>
            <span className="text-[9px] font-semibold bg-purple-100 dark:bg-[#DBB7F2]/20 text-purple-700 dark:text-[#DBB7F2] px-1.5 py-0.5 rounded uppercase">Beta</span>
          </div>
          ...
          {mainNav.map((item) => {
            ...
            return (
              <Link key={item.name} href={item.href} className={`flex items-center gap-3 px-3 py-1.5 rounded-md text-[13px] font-medium transition-colors ${isActive ? 'bg-gray-200 dark:bg-white/10 text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-white/5 hover:text-gray-900 dark:hover:text-white'}`}>
                <item.icon size={16} className={isActive ? 'text-purple-600 dark:text-[#DBB7F2]' : 'text-gray-400 dark:text-gray-500'} />
                {item.name}
              </Link>
            );
          })}
      ...
      <div className="p-4 space-y-4 mb-2">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-xs font-semibold text-gray-900 dark:text-white">
            <div className="w-4 h-4 rounded bg-[#DBB7F2] flex items-center justify-center">
               <span className="block w-2 h-2 bg-white dark:bg-[#0F0F13] rounded-sm" />
            </div>
            <span>5,941 <span className="text-gray-500 font-medium">/ 50,000 Text Credits</span></span>
          </div>
          <div className="h-1.5 w-full bg-gray-200 dark:bg-white/10 rounded-full overflow-hidden">
            <div className="h-full bg-purple-500 dark:bg-[#DBB7F2] rounded-full" style={{ width: '12%' }} />
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="h-1.5 w-full bg-gray-200 dark:bg-white/10 rounded-full overflow-hidden">
             <div className="h-full bg-purple-500 dark:bg-[#DBB7F2] rounded-full" style={{ width: '18%' }} />
          </div>
        </div>
